...
func RunTranscoder(n *core.LivepeerNode, orchAddr string, maxSessions int) {
	expb := backoff.NewExponentialBackOff()
	expb.MaxInterval = time.Minute
	expb.MaxElapsedTime = 0
	backoff.Retry(func() error {
		glog.Info("Registering transcoder to ", orchAddr)
		err := runTranscoder(n, orchAddr, maxSessions)
...
func runTranscoder(n *core.LivepeerNode, orchAddr string, maxSessions int) error {
	...
	r, err := c.RegisterTranscoder(ctx, &net.RegisterRequest{Secret: n.OrchSecret, MaxSessions: int64(maxSessions)})
	if err != nil {
...
func (h *lphttp) RegisterTranscoder(req *net.RegisterRequest, stream net.Transcoder_RegisterTranscoderServer) error {
	...
	h.orchestrator.ServeTranscoder(stream, int(req.MaxSessions))
	return nil
}
