...
func TestLastSegmentTimeout(t *testing.T) {
	initCensus("tst", "testid", "testversion", true)
	// defer func() {
	// 	shutDown <- nil
	// }()
	...
	SegmentFullyTranscoded(1, 1, "ps", true, SegmentTranscodeErrorUnknown)
	if sr := census.successRate(); sr != 1 {
		t.Fatalf("Success rate should be 1, not %f", sr)
	}
	...
	SegmentFullyTranscoded(1, 4, "ps", true, SegmentTranscodeErrorUnknown)
	...
	SegmentFullyTranscoded(2, 1, "ps", true, SegmentTranscodeErrorUnknown)
	...
	SegmentFullyTranscoded(3, 1, "ps", true, SegmentTranscodeErrorUnknown)
